...
Observando el gráfico de la función podemos ver que $f(1)=0$ y que $f(2)=-1$. El dominio de $f$ es $D=[0,3]$ y el rango corresponde al intervalo $[-1,2]$.
:::
...
Observemos que $f$ es una función lineal y $g$ una cuadrática, por lo que sus gráficos corresponden a una recta y una parábola, respectivamente.
...
Un recipiente rectangular para almacenamiento, con su parte superior abierta, tiene un volumen de 10 m<sup>3</sup>. La longitud de su base es el doble de su ancho. Si el material de la base cuesta 10 dólares por m<sup>2</sup> y el de los lados 6 dólares por m<sup>2</sup>, expresar el costo de fabricación en función del ancho de la base.
:::
...
Dado que el volumen es de 10 m<sup>3</sup>, tenemos que $h=\displaystyle \frac{5}{w^2}$. Con esta relación podemos expresar el área de la base $A_b$ de la caja y su área lateral $A_\ell$ en función de $w$.
...
$$C_b=10\, \frac{\text{dólares}}{\text{m}^2}\cdot (2w^2)\, \text{m}^2= 20w^2\, \text{dólares}.$$

$$C_\ell=6\, \frac{\text{dólares}}{\text{m}^2}\cdot \frac{30}{w}\, \text{m}^2= \frac{180}{w}\, \text{dólares}.$$
...
$$
-1\leq \operatorname{sen} x\leq 1 \quad \text { y }\quad  -1\leq \cos x\leq 1
$$
o, en términos de valor absoluto
$$
|\operatorname{sen} x|\leq 1 \quad \text{ y }\quad |\cos x|\leq 1.
$$

Además
$$
\operatorname{sen} x = 0 \quad\text{ si y sólo si }\quad x=n\pi, \,\,n\in\mathbb{Z}
$$
...
$$
\operatorname{sen}(x+2\pi)=\operatorname{sen} x\quad \text{ y }\quad \cos(x+2\pi)=\cos x
$$
para todo valor de $x$.

La función **tangente** está dada por $\displaystyle\tan x=\frac{\operatorname{sen} x}{\cos x}.$
...
| $y=f(cx)$|  comprimirse horizontalmente en un factor $c$    | 
| $y=f(x/c)$|  alargarse horizontalmente en un factor $c$ |
| $y=-f(x)$|  reflejarse respecto del eje $x$  |
| $y=f(-x)$|  reflejarse respecto del eje $y$  |
:::
...
Partiendo del gráfico de $y=f(x)=\sqrt{x}$ usar transformaciones para dibujar el de la función dada en cada caso.
...
con lo que la gráfica de $y=(1/a)^x$ se obtiene de la de $f$ **reflejándola con respecto al eje** $y$.
...
**(2)** $\displaystyle \log_a\left(\frac{x}{y}\right)=\log_a x-\log_a y$.
...
Utilizando las [leyes de los logaritmos](#propo-leyes-logaritmos) (1) y (3) podemos escribir
...
:::{.formula-box}
$$
\operatorname{sen}^{-1}(\operatorname{sen} x)=x, \quad \text{ para } \quad  -\tfrac{\pi}{2}\leq x\leq \tfrac{\pi}{2}
$$

y también

$$
\operatorname{sen}(\operatorname{sen}^{-1} x)=x, \quad \text{ para } \quad -1\leq x\leq 1.
$$
